...
function HomeContent() {
      ...
      const loadSharedFile = async () => {
        try {
          ...
          if (sharedFile) {
            ...
            const blob = new Blob([sharedFile.arrayBuffer], {
          ...
          if (result) {
            ...
            historyRequest.onsuccess = () => {
              ...
              histories.forEach((item) => {
                const historyFile = new File([item.data], item.name, {
                  type: item.type,
                })
  ...
  const saveToHistory = async (file: File, currentGain: number) => {
    const newHistory = [
      ...audioHistory.slice(0, historyIndex + 1),
      { file, gain: currentGain },
    ]
  ...
  const handleCrop = async () => {
    ...
    const croppedBlob = await cropAudio(
      audioFile,
      cropRegion.start,
      cropRegion.end
    )
    const croppedFile = new File([croppedBlob], 'cropped_audio.wav', {
      type: 'audio/wav',
    })
  ...
  const handleGainApply = async () => {
    ...
    setIsProcessing(true)
    const adjustedBlob = await adjustGain(
      audioFile,
      gain,
      cropRegion || undefined
    )
    const adjustedFile = new File([adjustedBlob], 'adjusted_audio.wav', {
      type: 'audio/wav',
    })
              ...
              <div className='mt-4 flex items-center space-x-4'>
                ...
                {cropRegion && (
                  <span className='text-sm text-gray-300'>
                    Sélectionné : {cropRegion.start.toFixed(2)}s -{' '}
                    {cropRegion.end.toFixed(2)}s
                  </span>
                )}
              </div>
            </div>
